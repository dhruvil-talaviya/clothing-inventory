...
import {
    ...
    FiTag, FiHash, FiRefreshCw, FiPercent
} from 'react-icons/fi';
...
const StatCard = ({ title, value, icon, color, onClick, badge, profitValue }) => {
    const c = {
    ...
    return (
        <button onClick={onClick} className="bg-[#1e293b] p-5 rounded-2xl border border-slate-800 shadow-lg hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 text-left w-full">
...
const AdminOverview = () => {
    ...
    const [stats,          setStats]          = useState({ today:0, month:0, year:0, total:0, chartData:[], pieData:[] });
    ...
    const calculateStats = (sales) => {
        ...
        let todaySum=0, monthSum=0, yearSum=0, totalSum=0;
        const chartMap={}, categoryMap={};
        sales.forEach(sale => {
            ...
            if (sd) {
                if (sd.getFullYear()===cy) {
                    yearSum += amt;
                    if (sd.getMonth()===cm) { monthSum += amt; if (sd.getDate()===cd) todaySum += amt; }
                }
        ...
        const pieColors = ['#6366f1','#ec4899','#f59e0b','#10b981','#8b5cf6'];
        const pieData = Object.keys(categoryMap).map((cat,i) => ({ name:cat, value:categoryMap[cat], color:pieColors[i%5] }));
        setStats({ today:todaySum, month:monthSum, year:yearSum, total:totalSum, chartData, pieData });
    };
    ...
    if (loading) return <div className="flex items-center justify-center h-64"><div className="text-slate-400 animate-pulse flex items-center gap-2"><FiRefreshCw className="animate-spin"/> Loading Dashboard...</div></div>;
    ...
    return (
        <div className="space-y-6 font-sans text-slate-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
                <StatCard title="Today's Sales"   value={stats.today} icon={<FiDollarSign/>} color="blue"    badge="Today"      onClick={() => handleCardClick('today')}/>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
                ...
                <div className="bg-[#1e293b] p-6 rounded-2xl border border-slate-800 shadow-xl flex flex-col" style={{height:'22rem'}}>
                    <h3 className="text-base font-bold text-white mb-2 flex items-center gap-2 shrink-0"><FiShoppingBag className="text-pink-400"/> Top Categories</h3>
                    <div className="flex-1 min-h-0">
                        {stats.pieData.length > 0 ? (
                            <ResponsiveContainer width="100%" height="100%">
                                <PieChart>
                                    <Pie data={stats.pieData} cx="50%" cy="45%" innerRadius={55} outerRadius={75} paddingAngle={4} dataKey="value" stroke="none">
                                        {stats.pieData.map((e,i) => <Cell key={i} fill={e.color}/>)}
                                    </Pie>
                                    <Tooltip formatter={(v,n) => [`${v} sales`,n]} contentStyle={{backgroundColor:'#0f172a',borderRadius:'8px',border:'1px solid #334155',fontSize:'12px'}}/>
                                    <Legend verticalAlign="bottom" height={36} iconType="circle" wrapperStyle={{fontSize:'11px'}}/>
                                </PieChart>
                            </ResponsiveContainer>
                        ) : (
                            <div className="h-full flex flex-col items-center justify-center text-slate-500 text-sm gap-2">
                                <FiShoppingBag size={28} className="opacity-20"/><span>No Category Data</span>
                            </div>
                        )}
                    </div>
                </div>
